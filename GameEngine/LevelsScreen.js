...
class LevelsScreen {
    ...
    goToWelcomeScreen() {
        ...
        if (window.ELEVATOR_TRANSITION) {
            console.log("Using elevator transition to return to welcome screen");
            ...
            console.log("No elevator transition available, showing welcome screen directly");
            const welcomeScreen = document.getElementById("welcomeScreen");
            if (welcomeScreen) {
                welcomeScreen.style.display = "flex";
            }
    ...
    async loadLevel(levelNum) {
        try {
            ...
            // First hide the levels screen
            this.hide();

            // Store the target level for loading
            window.targetLevelToLoad = levelNum;

            // Use elevator transition if available
            if (window.ELEVATOR_TRANSITION) {
                console.log("Using elevator transition to load level:", levelNum);
                window.ELEVATOR_TRANSITION.transition(() => {
                    // This callback is executed after doors close
                    this.actuallyLoadLevel(levelNum);
                });
            } else {
                console.log("No elevator transition available, loading level directly");
                this.actuallyLoadLevel(levelNum);
            }
        } catch (error) {
    ...
    // New method to handle the actual level loading after transition
    // Helper method to actually load the level
    actuallyLoadLevel(levelNum) {
        ...
        if (!window.gameEngine || !window.gameEngine.levelConfig) {
            console.log("No existing game engine or level config, starting fresh");
            startGame();
        } else {
            console.log("Using existing game engine to load level:", levelNum);
            // Load level with existing game engine
            window.gameEngine.levelConfig.currentLevel = levelNum;
            window.gameEngine.levelConfig.loadLevel(levelNum);

            // Switch music to game music
            if (window.AUDIO_MANAGER) {
    ...
    async goTogame() {
        ...
        // Use the elevator transition if available
        if (window.ELEVATOR_TRANSITION) {
            console.log("Using elevator transition to return to game");
            window.ELEVATOR_TRANSITION.transition(() => {
                this.resumeGameAftterTransition(levelToLoad);
            });
        } else {
            console.log("No elevator transition available, resuming game directly");
            this.resumeGameAftterTransition(levelToLoad);
        }
    }

    // Helper method to resume the game after transition
    resumeGameAftterTransition(levelToLoad) {
        if (window.gameEngine) {
            ...
            this.actuallyLoadLevel(levelToLoad);
        }
    }
}